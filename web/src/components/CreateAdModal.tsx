...
export function CreateAdModal() {
    ...
    useEffect(() => {
        axios('http://127.0.0.1:4000/games').then(response => {
    ...
    function handleSubmit(e: FormEvent) {
        ...
        if (!data.name || !data.game || !data.hourStart || !data.hourEnd || !data.yearsPlaying || !data.discord)
            return;

        axios.post('http://127.0.0.1:4000/ads', {
            gameId: data.game,
            name: data.name,
        ...
        <Dialog.Content className="fixed bg-[#2A2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[480px] shadow-lg shadow-black/25">
            ...
            <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-4">
                <div className="flex flex-col gap-2">
                    ...
                    <select defaultValue="" name='game' id="game" className="bg-zinc-900 py-3 px-4 rounded text-sm placeholder:text-zinc-500 appearance-none" required >
